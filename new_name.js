...
async function analyzeAllTestsAndSendResults(chatId) {
    try {
        ...
        context += `Предложи 1 наиболее подходящие технику релаксации из представленных Неспецифические техники релаксации
...
function getTestSpecificInstructions(testNumber) {
    ...
    instructions += `1. Оцени психоэмоциональное состояние пользователя. ТЕХНИКИ РЕЛАКСАЦИИ ВЫДАВАТЬ НЕ НУЖНО\n`;
    ...
    switch(testNumber) {
        case 'test1':
            instructions += ` Тест называется "Акцентуация характера". тебе нужно оценить состояние человека по данным его ответов, и выдать только анализ состояния. Так же отпределить тип (Демонстративный тип,  Застревающий тип, Педантичный тип, Возбудимый тип и тд). Только описание психоэмоциональное состояние пользователя. НИКАКИЕ ТЕХНИКИ РЕАКСАЦИИ ВЫДАВАТЬ НЕ НУЖНО!!!! \n`;
            break;
        case 'test2':
            instructions += `Тест называется "Тест аудиал, визуал, кинестетик (диагностика доминирующей перцептивной
                модальности С. Ефремцева / методика на восприятие)". тебе нужно оценить состояние человека по данным его ответов, и выдать только анализ состояния. Так же опредеить тип (Аудиальный тип, Аудиальный тип и тд). Только описание психоэмоциональное состояние пользователя. НИКАКИЕ ТЕХНИКИ РЕАКСАЦИИ ВЫДАВАТЬ НЕ НУЖНО!!!! \n`;
            break;
        case 'test3':
             instructions += `Тест называется "Госпитальная Шкала Тревоги и Депрессии (HADS)". тебе нужно оценить состояние человека по данным его ответов, и выдать только анализ состояния. Только описание психоэмоциональное состояние пользователя. НИКАКИЕ ТЕХНИКИ РЕАКСАЦИИ ВЫДАВАТЬ НЕ НУЖНО!!!! \n`;
            break;
        case 'test4':
            instructions += `Тест называется "Опросник выраженности психопатологической симптоматики
            (Symptom checklist-90-revised)". тебе нужно оценить состояние человека по данным его ответов, и выдать только анализ состояния. Только описание психоэмоциональное состояние пользователя. НИКАКИЕ ТЕХНИКИ РЕАКСАЦИИ ВЫДАВАТЬ НЕ НУЖНО!!!! \n`;
            break;
    }
